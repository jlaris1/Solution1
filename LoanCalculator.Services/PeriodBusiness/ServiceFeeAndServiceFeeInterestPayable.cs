...
namespace LoanAmort_driver.PeriodBusiness
{
    public class ServiceFeeAndServiceFeeInterestPayable
    {
            ...
            try
            {
                ...
                else if (scheduleInputs.ApplyServiceFeeInterest == 1 || scheduleInputs.ApplyServiceFeeInterest == 3)
                {
                    ...
                    else
                    {
                        ...
                        //Calculating total service fee payable.
        ...
        private static double GetAccruedValue(List<OutputGrid> paymentDetailIn, DateTime accruedInterestDate, DateTime effectiveDate, string dataType, LoanDetails scheduleInputs)
        {
            ...
            try
            {
                sbTracing.AppendLine("Enter:Inside ServiceFeeAndServiceFeeInterestPayable Class file and method name GetAccruedValue(List<OutputGrid> paymentDetailIn, DateTime accruedInterestDate, DateTime effectiveDate, string dataType, LoanDetails scheduleInputs).");
                ...
                if (dateFrom < accruedInterestDate)
                {
                    ...
                    for (int i = 0; i < paymentDetailIn.Count; i++)
                    {
                        ...
                        if (paymentDetailIn[i].PaymentDate == accruedInterestDate)
                        {
                            ...
                            if (paymentDetailIn[i].Flags != (int)Models.Constants.FlagValues.ManagementFee && paymentDetailIn[i].Flags != (int)Models.Constants.FlagValues.MaintenanceFee)
                            {
                    ...
                    if (doPartialPeriod)
                    {
                        ...
                        sbTracing.AppendLine("Inside ServiceFeeAndServiceFeeInterestPayable class, and CalculateServiceFeeAndInterestPayable() method. Calling method : PeriodBusiness.DaysInPeriod.GetNumberOfDays(dateFrom, dateTo, scheduleInputs, false);");
                        long periodDayCount = PeriodBusiness.DaysInPeriod.GetNumberOfDays(dateFrom, dateTo, scheduleInputs, false, false);
                ...
                sbTracing.AppendLine("Exit:From ServiceFeeAndServiceFeeInterestPayable Class file and method name GetAccruedValue(List<OutputGrid> paymentDetailIn, DateTime accruedInterestDate, DateTime effectiveDate, string dataType, LoanDetails scheduleInputs)");
                return returnVal;
            }
            catch (Exception ex)
            {
        ...
        public static double GetInterestAccrued(List<OutputGrid> paymentDetailIn, DateTime accruedInterestDate, DateTime effectiveDate, LoanDetails scheduleInputs)
        {
            ...
            try
            {
                sbTracing.AppendLine("Enter:Inside ServiceFeeAndServiceFeeInterestPayable Class file and method name GetInterestAccrued(List<OutputGrid> paymentDetailIn, DateTime accruedInterestDate, DateTime effectiveDate, LoanDetails scheduleInputs).");
                sbTracing.AppendLine("Parameter values are : accruedInterestDate = " + accruedInterestDate + ", effectiveDate = " + effectiveDate);
                ...
                sbTracing.AppendLine("Exit:From ServiceFeeAndServiceFeeInterestPayable Class file and method name GetInterestAccrued(List<OutputGrid> paymentDetailIn, DateTime accruedInterestDate, DateTime effectiveDate, LoanDetails scheduleInputs)");
                return returnVal;
            }
            catch (Exception ex)
            {
        ...
        public static double GetAccruedPrincipal(List<OutputGrid> paymentDetailIn, DateTime accruedInterestDate, DateTime effectiveDate, LoanDetails scheduleInputs, double loanAmountCalc)
        {
            ...
            try
            {
                sbTracing.AppendLine("Enter:Inside ServiceFeeAndServiceFeeInterestPayable Class file and method name GetAccruedPrincipal(List<OutputGrid> paymentDetailIn, DateTime accruedInterestDate, DateTime effectiveDate, LoanDetails scheduleInputs, double loanAmountCalc).");
                ...
                sbTracing.AppendLine("Exit:From ServiceFeeAndServiceFeeInterestPayable Class file and method name GetAccruedPrincipal(List<OutputGrid> paymentDetailIn, DateTime accruedInterestDate, DateTime effectiveDate, LoanDetails scheduleInputs, double loanAmountCalc)");
                return returnVal;
            }
            catch (Exception ex)
            {

...
namespace LoanAmort_driver.PeriodBusiness
{
    public class ManagementAndMaintenanceFeeCalc
    {
        ...
        public static DateTime EffectiveDateOfFee(int frequency, int delay, DateTime effectiveDate, List<DateTime> scheduledPaymentDates, List<Models.AdditionalPaymentRecord> allPaymentDates)
        {
            ...
            try
            {
                sbTracing.AppendLine("Enter:Inside Business Class ManagementAndMaintenanceFeeCalc and method name EffectiveDateOfFee(int frequency, int delay, DateTime effectiveDate, List<DateTime> scheduledPaymentDates, List<DateTime> allPaymentDates).");
                ...
                sbTracing.AppendLine("Exit:From Business Class ManagementAndMaintenanceFeeCalc and method name EffectiveDateOfFee(int frequency, int delay, DateTime effectiveDate, List<DateTime> scheduledPaymentDates, List<DateTime> allPaymentDates)");
        ...
        {
            StringBuilder sbTracing = new StringBuilder();
            sbTracing.AppendLine("Enter:Inside Business Class ManagementAndMaintenanceFeeCalc and method name PayableAmountAsPerMonthAndLoan(LoanDetails scheduleInput, DateTime date, List<OutputGrid> outputGrid, double payableFee, bool isManagementFeeCalculation).");
            sbTracing.AppendLine("Parameter values are : date = " + date + ", payableFee = " + payableFee + ", isManagementFeeCalculation = " + isManagementFeeCalculation);
            ...
            try
            {
                ...
                DateTime lastDate = scheduleInput.InputRecords[scheduleInput.InputRecords.Count - 1].EffectiveDate == DateTime.MinValue ?
                ...
                bool isExtend = lastDate < date;

                //Calculate fee for per month
                if (maxFeePerMonth != 0)
                {
                    double paidFeePerMonth = isManagementFeeCalculation ? copyOutputGrid.Where(o => o.PaymentDate.Month == date.Month && o.PaymentDate.Year == date.Year).Sum(o => o.ManagementFee) :
                                                                            copyOutputGrid.Where(o => o.PaymentDate.Month == date.Month && o.PaymentDate.Year == date.Year).Sum(o => o.MaintenanceFee);
                    ...
                    if (isExtend && lastDate.Month == date.Month && lastDate.Year == date.Year)
                    {
                        double deductableAmount = isManagementFeeCalculation ? copyOutputGrid[copyOutputGrid.FindLastIndex(o => o.PaymentDate == lastDate) - 1].ManagementFeePastDue :
                                                    copyOutputGrid[copyOutputGrid.FindLastIndex(o => o.PaymentDate == lastDate) - 1].MaintenanceFeePastDue;
                        paidFeePerMonth = Math.Round((paidFeePerMonth - deductableAmount), 2, MidpointRounding.AwayFromZero);
                    }
                ...
                if (maxFeePerLoan != 0)
                {
                    double paidFeePerLoan = isManagementFeeCalculation ? copyOutputGrid.Sum(o => o.ManagementFee) : copyOutputGrid.Sum(o => o.MaintenanceFee);
                    ...
                    if (isExtend)
                    {
                        double deductableAmount = isManagementFeeCalculation ? copyOutputGrid[copyOutputGrid.FindLastIndex(o => o.PaymentDate == lastDate) - 1].ManagementFeePastDue :
                                                    copyOutputGrid[copyOutputGrid.FindLastIndex(o => o.PaymentDate == lastDate) - 1].MaintenanceFeePastDue;
                        paidFeePerLoan = Math.Round((paidFeePerLoan - deductableAmount), 2, MidpointRounding.AwayFromZero);
                    }
                ...
                sbTracing.AppendLine("Exit:From Business Class ManagementAndMaintenanceFeeCalc and method name PayableAmountAsPerMonthAndLoan(LoanDetails scheduleInput, int month, List<OutputGrid> outputGrid, double payableFee, bool isManagementFeeCalculation)");
                return payableFee;
            }
            catch (Exception ex)
            {
                                        ...
                                        int inputRowNumber, bool isForAdditionalPayment, bool isExtendedRow, bool isEarlyPayOff, ref int managementFeeTableIndex, ref int lastManagementFeeChargedIndex)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    // finding the index uning schedule date from MaintenanceFeeAssessment table.
                    int mgmgtIndex = outputSchedule.ManagementFeeAssessment.FindIndex(o => o.AssessmentDate == inputGridDate);
                    if (scheduleInput.ManagementFeeBasis != (int)Constants.FeeBasis.ActualPayment && scheduleInput.ManagementFeeBasis != (int)Constants.FeeBasis.ScheduledPayment)
                    {
                        while (managementFeeIndex < outputSchedule.ManagementFeeAssessment.Count && managementFeeIndex <= mgmgtIndex
                    ...
                    for (int index = previousChargedIndex + 1; index < outputSchedule.ManagementFeeAssessment.Count && index <= mgmgtIndex && (outputSchedule.ManagementFeeAssessment[index].AssessmentDate <= inputGridDate); index++)
                    {
                                        ...
                                        int inputRowNumber, bool isForAdditionalPayment, bool isExtendedRow, bool isEarlyPayOff, ref int maintenanceFeeTableIndex, ref int lastMaintenanceFeeChargedIndex)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    for (int index = previousChargedIndex + 1; index < outputSchedule.MaintenanceFeeAssessment.Count && index <= maintenanceIndex && (outputSchedule.MaintenanceFeeAssessment[index].AssessmentDate <= inputGridDate); index++)
                    {

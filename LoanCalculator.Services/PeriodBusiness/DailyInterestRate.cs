...
namespace LoanAmort_driver.PeriodBusiness
{
    public class DailyInterestRate
    {
        ...
        /// <returns></returns>
        public static double CalculateDailyInterestRate(string daysInYear, string daysInMonth, DateTime startDate, DateTime endDate, double periodicInterestRate, DateTime loanTakenDate, int interestDelay, LoanDetails scheduleInput, bool isScheduleInput)
        {
            StringBuilder sbTracing = new StringBuilder();
            double dailyInterestRate = 0;
            try
            {
                sbTracing.AppendLine("Enter:Inside DailyInterestRate class and Method Name CalculateDailyInterestRate(string daysInYear, string daysInMonth, DateTime startDate, DateTime endDate, double periodicInterestRate, DateTime loanTakenDate, int interestDelay).");

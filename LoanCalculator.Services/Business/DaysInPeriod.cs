...
namespace LoanAmort_driver.Business
{
    public class DaysInPeriod
    {
        ...
        /// <returns></returns>
        public static int GetNumberOfDays(DateTime startDate, DateTime endDate, LoanDetails scheduleInput, bool isScheduleInput, bool isForTotalPayment)

        {
            ...
            try
            {
                sbTracing.AppendLine("Enter:Inside DaysInPeriod class and Method Name  GetNumberOfDays(DateTime startDate, DateTime endDate, LoanDetails scheduleInput, bool isScheduleInput).");
                sbTracing.AppendLine("Parameter values are :  startDate = " + startDate + ", endDate = " + endDate + ", isScheduleInput = " + isScheduleInput);

                if ((daysInMonth.ToLower() == Constants.Periodic || daysInMonth == Constants.Thirty) && daysInYear.ToLower() == Constants.Actual)//This condition will throw an error as when no. of days in month is 30, then days in year must be a numeric value.
                {
                    throw new ArgumentException("Invalid Days in Year Value");
                }

                if (daysInMonth.ToLower() == Constants.Actual && daysInYear.ToLower() == Constants.Actual) // We determine dates may be either under non-leap year or leap year to calculate factor.
                {
                    int nonLeapYearDays;
                    int leapYearDays;
                ...
                else  //This is the condition where no. of days in year is a numeric value, irrespective of no. of days in month.
                {
                    if (daysInMonth == Constants.Thirty)    //In this condition factor will be evaluting as per fixed method.
                    {
                ...
                sbTracing.AppendLine("Exist:From DaysInPeriod class and Method Name GetNumberOfDays(string daysInYear, string daysInMonth, DateTime startDate, DateTime endDate)");
                return days;
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Determine the no of days in period.
        /// </summary>
        /// <param name="startDate"></param>
        /// <param name="endDate"></param>
        ...
        /// <returns></returns>
        private static int NumberOfDaysInPeriod(DateTime startDate, DateTime endDate, LoanDetails scheduleInput, bool isScheduleInput, bool isForTotalPayment)
        {
            /*

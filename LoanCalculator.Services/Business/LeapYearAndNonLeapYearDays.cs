    ...
    public class LeapYearAndNonLeapYearDays
    {
        ...
        public static void GetLeapAndNonLeapYearDays(DateTime startDate, DateTime endDate, out int nonLeapYearDays, out int leapYearDays)
        {
            ...
            try
            {
                ...
                //This condition checks if both dates are in same year, then days will lay either in leap year or in non-leap year.
                ...
                //When year of both dates are not same.
                else
                {
                    //It will check if start date year is in leap year, if yes then subtract start date from last date of year, and no. of days will be part of leap year.
                    if ((startDate.Year) % 4 == 0)
                    {
                    ...
                    //It will check if end date year is in leap year, if yes then subtract last date of start date year from end date, and add no. of days in leap year.
                    if ((endDate.Year) % 4 == 0)
                    {

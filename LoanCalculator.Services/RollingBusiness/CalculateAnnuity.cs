...
namespace LoanAmort_driver.RollingBusiness
{
    public class CalculateAnnuity
    {
        ...
        public static double CalcateAnnuityWithLongerFirstPeriod(double firstRegularInterestPayment, double firstInterestPaymentPaid, double regularPaymentAsPerPaymentPeriod,
                                                             getScheduleInput scheduleInputs, bool calculateServiceFeeTotal, double principalServiceFee, double loanAmountCalc)
        {
            StringBuilder sbTracing = new StringBuilder();
            try
            {
                sbTracing.AppendLine("Enter:Inside class CalculateAnnuity and method name CalcateAnnuityWithLongerFirstPeriod(double firstRegularInterestPayment, double firstInterestPaymentPaid, double regularPaymentAsPerPaymentPeriod, getScheduleInput scheduleInputs, bool calculateServiceFeeTotal, double principalServiceFee, double loanAmountCalc).");
                ...
                int nPer = scheduleInputs.BalloonPayment > 0 && !calculateServiceFeeTotal ? scheduleInputs.InputRecords.Count - 1 : scheduleInputs.InputRecords.Count;
                //Iterate while difference reaches allowed value:
                do
                {
                    for (int i = 1; i < nPer; i++)
                    {
                        sbTracing.AppendLine("Inside CalculateAnnuity class, and CalcateAnnuityWithLongerFirstPeriod() method. Calling method : PeriodicInterestRate.GetPeriodicInterestRate(scheduleInputs.DaysInYearBankMethod, scheduleInputs.DaysInMonth, scheduleInputs.InputRecords[i - 1].DateIn, scheduleInputs.InputRecords[i].DateIn, Convert.ToDouble(scheduleInputs.InterestRate, false, true, true));");
                        double currentInterestPayment = PeriodicInterestAmount.CalcAccruedInterestWithTier(scheduleInputs, scheduleInputs.InputRecords[i - 1].DateIn, scheduleInputs.InputRecords[i].DateIn, scheduleInputs.InputRecords[i - 1].DateIn, currentPrincipal, false, true, true);
                        ...
                        if (i == (nPer - 1))
                        {
                            double paymentLast = currentPrincipal + currentInterestPayment + interestDiff - (calculateServiceFeeTotal ? 0 : (scheduleInputs.Residual > 0 ? scheduleInputs.Residual : scheduleInputs.BalloonPayment));
                            if (paymentLast < 0)
                            {
                                paymentLast = 0;
                            }
                ...
                sbTracing.AppendLine("Exist:From class CalculateAnnuity and method name CalcateAnnuityWithLongerFirstPeriod(double regularInterestRate, double firstPeriodInterestRate, double regularPaymentAsPerPaymentPeriod, getScheduleInput scheduleInputs, bool calculateServiceFeeTotal, double principalServiceFee, double loanAmountCalc).");
                return paymentOne;
            }
            catch (Exception ex)
            {
